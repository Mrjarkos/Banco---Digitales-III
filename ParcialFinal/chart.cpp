    ...
    zz_y(1)
{
    ...
    xx_series->attachAxis(m_axisY);

    yy_series->attachAxis(m_axisX);
    yy_series->attachAxis(m_axisY);

    zz_series->attachAxis(m_axisX);
    zz_series->attachAxis(m_axisY);
...
void Chart::handleTimeout()
{
    ...
    if(xx_y!=0){xx_series->append(m_x, xx_y);}
    if(yy_y!=0){yy_series->append(m_x, yy_y);}
    if(zz_y!=0){zz_series->append(m_x, zz_y);}
